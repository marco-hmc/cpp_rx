    ...
    namespace RTTI {
        ...
        struct TypeInfo {
            ...
            inline intptr_t CastTo(intptr_t aPtr, typeId_t aTypeId) const {
                ...
                while (true) {
                    typeId_t size =
                        *reinterpret_cast<const typeId_t*>(data + byteIndex);
                    byteIndex += sizeof(typeId_t);

                    for (typeId_t i = 0; i < size;
                         i++, byteIndex += sizeof(typeId_t)) {
                        if (*reinterpret_cast<const typeId_t*>(
                                data + byteIndex) == aTypeId)
                            return aPtr + offset;
                    }

                    offset =
                        *reinterpret_cast<const ptrdiff_t*>(data + byteIndex);
            ...
            KCL_FORCEINLINE bool operator==(const TypeInfo& anOther) const {
                return GetTypeId() == anOther.GetTypeId();
            }
            KCL_FORCEINLINE bool operator!=(const TypeInfo& anOther) const {
                return GetTypeId() != anOther.GetTypeId();
            }
        ...
        KCL_FORCEINLINE const TypeInfo* GetTypeInfo() {
            typedef typename std::decay<typename std::remove_cv<T>::type>::type
                Type;
...
#pragma pack(push, 1)
        ...
        struct BaseTypeData<FirstBase, SecondBase, Next...> {
            ...
            void FillBaseTypeData(ptrdiff_t aOffset, typeId_t& outHeadSize) {
                myFirst.template FillBaseTypeData<Derived>(
                    ComputePointerOffset<Derived, FirstBase>(), outHeadSize);
        ...
        struct BaseTypeData<Base> {
            template <typename Derived>
            void FillBaseTypeData(ptrdiff_t aOffset, typeId_t& outHeadSize) {
                const TypeData<Base>* baseTypeId =
                    (TypeData<Base>*)(GetTypeInfo<Base>::Get()->GetTypeData());
                ...
                while (offset != 0) {
                    ...
                    *reinterpret_cast<ptrdiff_t*>(myData + byteIndex) =
                        offset + aOffset;
                    byteIndex += sizeof(ptrdiff_t);
            ...
            char myData[sizeof(TypeData<Base>) - sizeof(ptrdiff_t) -
                        sizeof(typeId_t)];
        };
        ...
        struct TypeDataImpl {
            TypeDataImpl() {
                myTypeId = GenerateId();
                myBaseTypeData.template FillBaseTypeData<Type>(
                    0 /* No offset with first base */, mySize);
        ...
        struct TypeDataImpl<Type> {
            TypeDataImpl()
                : mySize(1), myTypeId(GenerateId()), myEndMarker(0) {}
...
#define KCL_RTTI_TYPEINFO(TYPE)                                         \
    ...
    struct GetTypeInfo<TYPE> {                                          \
        static const KCL::RTTI::TypeInfo* Get() {                       \
            static TypeInfoImpl<TYPE> ourInstance = {{#TYPE},           \
                                                     TypeData<TYPE>()}; \
...
#define KCL_RTTI_REGISTER(...)                            \
    namespace KCL {                                       \
        namespace RTTI_Private {                          \
            template <>                                   \
            struct TypeData<KCL_FIRST_ARG(__VA_ARGS__)>   \
                : public TypeDataImpl<__VA_ARGS__> {};    \
...
#define KCL_RTTI_IMPL()                                                     \
                                                                            \
    virtual intptr_t KCL_RTTI_DynamicCast(KCL::RTTI::typeId_t aOtherTypeId) \
        const {                                                             \
        typedef std::remove_pointer<decltype(this)>::type ObjectType;       \
        return KCL::RTTI::template GetTypeInfo<ObjectType>()->CastTo(       \
            (intptr_t)this, aOtherTypeId);                                  \
    }                                                                       \
    virtual const KCL::RTTI::TypeInfo* KCL_RTTI_GetTypeInfo() const {       \
        typedef std::remove_pointer<decltype(this)>::type ObjectType;       \
        return KCL::RTTI::template GetTypeInfo<ObjectType>();               \
    }                                                                       \
    virtual const char* KCL_RTTI_GetTypeName() const {                      \
